...
public class Lab0 {
	...
	public static int lastTwoDigits(int n) {
		int finalTwoDigitsInt;
		if (n > 10 || n < -10) {
			String nInString = Integer.toString(n);
			String finalTwoDigitsString = nInString.substring((nInString.length() - 2), (nInString.length()));
			finalTwoDigitsInt = Integer.parseInt(finalTwoDigitsString);
		} else {
			finalTwoDigitsInt = n;
		}
		return finalTwoDigitsInt;
	}
	...
	public static double windChill(double airTemp, double windSpeed) {
		...
		if (airTemp <= 0 && windSpeed >= 5) {
			final double CONSTANT_A = 13.12;
			final double CONSTANT_B = 0.6215;
			final double CONSTANT_C = 0.3965;
			final double CONSTANT_D = 11.37;
			final double CONSTANT_E = 0.16;
			returnValue = CONSTANT_A + CONSTANT_B * airTemp
					+ (CONSTANT_C * airTemp - CONSTANT_D) * Math.pow(windSpeed, CONSTANT_E);
		}
		return returnValue;
	}
	...
	public static int flipBit(int x) {
		...
		for (int i = 0; i < xString.length(); i++) {
			if (xString.substring(i, i + 1).equals("1")) {
				newBits += 0;
			} else if (xString.substring(i, i + 1).equals("0")) {
				newBits += 1;
			} else {
				throw new BadBitException("Invalid");
			}
		}
		newBitsInt = Integer.parseInt(newBits);
		return newBitsInt;
	...
	public static int compareTo(Range r1, Range r2) {
		int returnValue = 0;
		double minR1 = r1.getMinimum();
		double maxR1 = r1.getMaximum();
		double minR2 = r2.getMinimum();
		double maxR2 = r2.getMaximum();

		if (Math.abs((maxR1) - (minR1)) == Math.abs((maxR2) - (minR2))) {
			returnValue = 0;
		} else if (Math.abs((maxR1) - (minR1)) < Math.abs((maxR2) - (minR2))) {
			returnValue = -1;
		} else {
			returnValue = 1;
		}
...
//		double firstNum = r.getMinimum();
//		double lastNum = r.getMaximum();
//		String displayRange = "";
//			for (double i = firstNum; i <= lastNum; i++) {
//				if (i != lastNum) {
//					displayRange = displayRange + i + ", ";
//				}
//				if (i == lastNum) {
//					displayRange = displayRange + i;
//				}
...
//		return displayRange;
	...
	public static char middleChar(String s) {
		...
		if (s.length() >= 1) {
			...
			midChar = s.charAt((charCounter / 2));
		} else {
	...
	public static void main(String[] args) {
		// String testing = alternatingCaps("Toronto");
