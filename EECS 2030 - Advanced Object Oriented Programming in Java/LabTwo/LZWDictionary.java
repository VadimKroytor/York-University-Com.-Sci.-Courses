...
public class LZWDictionary {
	...
	public LZWDictionary(String characters) {
		...
		for (int i = 0; i <= characters.length() - 1; i++) {
			String entry = characters.substring(i, i + 1);
			if (!(map.containsKey(entry))) {
				this.list.add(entry);
				this.map.put(entry, counter);
				counter++;
			}
		}
	}
	...
	public static void main(String[] args) {
		/*LZWDictionary testing = new LZWDictionary("wowzas");
		testing.add("woz");
		testing.add("woz");
		testing.add("woza");
		testing.add("wosz");
		System.out.println(testing);
		System.out.println(testing.get(7));
		*/
	}
	...
	public void reset() {
		...
		while (iterator.hasNext()) {
			...
			if (!(this.list.get(this.list.indexOf(i)).length() == 1)) {
				iterator.remove();
				this.map.remove(i);
			}
		}
	...
	public String get(int index) {
		...
		if (index < 0 || index > this.map.size()) {
			throw new IndexOutOfBoundsException("Not a valid index entry");
		}
	...
	public int add(String entry) {
		...
		if (!(this.map.containsKey(entry))) {
			this.map.put(entry, size);
			this.list.add(entry);
			returnValue = size;
		}
		else {
			returnValue = this.map.get(entry);
		}
		return returnValue;
 	}
